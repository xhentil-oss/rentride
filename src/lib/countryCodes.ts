// International dialing codes for the phone-prefix dropdown on the booking form.
// Flag emoji is derived from the ISO-3166 alpha-2 code (no hardcoded emojis).
// Albania + Kosovo are pinned first as the most common local prefixes.
...
/** ISO-3166 alpha-2 → flag emoji (e.g. "AL" → 🇦🇱). */
export const flagEmoji = (iso2: string): string =>
  /^[A-Za-z]{2}$/.test(iso2)
    ? iso2.toUpperCase().replace(/./g, (c) => String.fromCodePoint(127397 + c.charCodeAt(0)))
    : "🏳️";

// Pinned (local / most frequent) first, then the rest alphabetically by name.
export const COUNTRY_CODES: CountryCode[] = [
  { name: "Shqipëri", iso2: "AL", dial: "+355" },
  { name: "Kosovë", iso2: "XK", dial: "+383" },
